...
import { IsOptional, IsString, IsUUID, IsEnum, IsArray, ValidateNested, IsDateString, IsNotEmpty } from 'class-validator';
...
class FiltersDTO {
    ...
    @IsOptional()
    @IsEnum(CONTEXT, { each: true, message: 'context must be one of: user,cohort' })
    @IsString()
    context?: string;
    ...
    @IsOptional()
    @IsEnum(STATE, { each: true, message: 'state must be one of: publish,draft,archived' })
    @IsString()
    state?: string;
    ...
    @IsOptional()
    @IsEnum(ACTIONTYPE, { each: true, message: 'action_name must be one of the following values: reassign, delete' })
    @IsString()
    action_name?: string;

    @ApiProperty({
        example: [STATUS.COMPLETE, STATUS.INCOMPLETE, STATUS.REJECTED],
        description: 'Array of status values: complete, incomplete, rejected'
    })
    @IsOptional()
    @IsArray()
    @IsEnum(STATUS, {
        each: true,
        message: 'Status must be one of: complete, incomplete, rejected',
    })
    status?: string[];
